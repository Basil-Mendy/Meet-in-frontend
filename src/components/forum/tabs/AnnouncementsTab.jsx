        ...
        <div className="bg-white border rounded-lg p-4 hover:shadow-md transition">
            ...
            <div className="flex items-center justify-between text-xs text-gray-500">
                <div className="flex gap-4">
                    ...
                    {announcement.announcement_type === "EMAIL" && (
                        <span>{announcement.email_recipients?.length || 0} recipients</span>
                    )}
                </div>
...
export default function AnnouncementsTab({ forumId, isAdmin }) {
    ...
    useEffect(() => {
        fetchAnnouncements();
        markAllAsRead(); // Auto-mark all active announcements as read
    }, [showArchived]);

    const markAllAsRead = async () => {
        try {
            await api.post(`forums/${forumId}/announcements/mark_all_as_read/`);
        } catch (err) {
            // Silently fail if marking fails (not critical)
            console.debug("Mark all as read action completed or not needed");
        }
    };
    ...
    const handleArchive = (announcementId) => {
        fetchAnnouncements(); // Reload to refresh archive status
    };
            ...
            <div className="flex items-center justify-between">
                ...
                {isAdmin && (
                    <button
                        ...
                        className="px-2 py-1 text-xs sm:px-4 sm:py-2 sm:text-sm bg-blue-600 text-white rounded-lg hover:bg-blue-700 whitespace-nowrap"
                    >
                        + Create Announcement
                    </button>
                )}
